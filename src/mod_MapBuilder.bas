...
Public Function BuildMapHtml(ByVal jsonStr As String) As String
    ...
    ' --- Label visibility: show company name when map width <= 5 miles ---
    ' Handles opacity fading to match marker state when a selection is active.
    ...
    AppendLine sb, "  var show = widthMiles <= 5;"
    AppendLine sb, "  var sliderVal = selectedMarker ? parseFloat(document.getElementById('radiusSlider').value) : 0;"
    AppendLine sb, "  markers.forEach(function(m, i) {"
    AppendLine sb, "    if (show) {"
    AppendLine sb, "      m.openTooltip();"
    AppendLine sb, "      var tt = m.getTooltip();"
    AppendLine sb, "      var el = tt && tt.getElement();"
    AppendLine sb, "      if (el) {"
    AppendLine sb, "        el.style.color = getFieldColor(companies[i][currentField]);"
    AppendLine sb, "        if (selectedMarker) {"
    AppendLine sb, "          if (m === selectedMarker) {"
    AppendLine sb, "            el.style.opacity = '1';"
    AppendLine sb, "          } else {"
    AppendLine sb, "            var dist = haversine(selectedCompany.Lat, selectedCompany.Lon, companies[i].Lat, companies[i].Lon);"
    AppendLine sb, "            el.style.opacity = dist <= sliderVal ? '1' : '0.15';"
    AppendLine sb, "          }"
    AppendLine sb, "        } else {"
    AppendLine sb, "          el.style.opacity = '1';"
    AppendLine sb, "        }"
